...
export default function FileManagerFilters({
  ...
  const renderLabel = filters.option.length ? filters.option.slice(0, 2).join(',') : 'All type';
  ...
  const handleFilterType = useCallback(
    (newValue) => {
      const checked = filters.option.includes(newValue)
        ? filters.option.filter((value) => value !== newValue)
        : [...filters.option, newValue];
      onFilters('option', checked);
    },
        ...
        <Stack spacing={2.5}>
          ...
          >
            {typeOptions.map((type) => {
              const selected = filters.option.includes(type);
